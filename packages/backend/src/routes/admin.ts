...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const { action } = req.body as { action: 'archive' | 'restore' | 'delete' };
      ...
      switch (action) {
        ...
        case 'restore':
          ...
          break;
        case 'delete':
          await Database.query(`DELETE FROM project_files WHERE project_id = $1`, [id]);
          await Database.query(`DELETE FROM project_packages WHERE project_id = $1`, [id]);
          await Database.query(`DELETE FROM project_module_dependencies WHERE project_id = $1`, [id]);
...
router.post(
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const { action } = req.body as { action: 'promote' | 'demote' | 'disable' | 'enable' };
      ...
      switch (action) {
        ...
        case 'demote':
          ...
          break;
        case 'disable':
          await Database.query(`UPDATE users SET is_active = false WHERE id = $1`, [id]);
          break;
        case 'enable':
          await Database.query(`UPDATE users SET is_active = true WHERE id = $1`, [id]);
          break;
        default:
          return res.status(400).json({ success: false, error: 'Unsupported action' });
      }
