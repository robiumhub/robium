    ...
    for (const project of projects) {
      ...
      const result = (await Database.query(
        ...
        INSERT INTO projects (
          name, description, version, author, maintainer_email, license, category, type,
          is_template, config, metadata, workspace_path, source_path, config_path, status, owner_id
      ...
      projectIds.set(project.name, result.rows[0].id);
      console.log(
        `    ✅ Project ${project.name} added/updated with ID: ${result.rows[0].id}`
      );
    }
      ...
      for (let i = 0; i < project.module_dependencies.length; i++) {
        ...
        if (moduleId) {
          await Database.query(
            `
            INSERT INTO project_module_dependencies (project_id, module_id, dependency_type, order_index)
            VALUES ($1, $2, $3, $4)
        ...
        for (const pkg of modulePackagesResult.rows) {
          await Database.query(
            ...
            ON CONFLICT (project_id, package_id) DO UPDATE SET
              ...
              order_index = EXCLUDED.order_index
          `,
            [projectId, pkg.id, true, packageIndex]
          );
