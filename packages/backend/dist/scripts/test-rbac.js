...
async function testRBAC() {
    ...
    try {
        ...
        const adminResponse = await (0, node_fetch_1.default)(`${BASE_URL}/auth/signup`, {
            method: 'POST',
            headers: {
        ...
        const userResponse = await (0, node_fetch_1.default)(`${BASE_URL}/auth/signup`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const adminDashboardResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/dashboard`, {
            method: 'GET',
            headers: {
        ...
        if (adminDashboardResponse.ok && adminDashboardData.success) {
            ...
            console.log('   - Permissions count:', adminDashboardData.data?.permissions?.length);
        }
        else {
        ...
        console.log('4️⃣ Testing regular user access to admin dashboard (should fail)...');
        const userDashboardResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/dashboard`, {
            method: 'GET',
            headers: {
        ...
        if (userDashboardResponse.status === 403) {
            ...
            console.log('❌ Regular user incorrectly allowed admin dashboard access:', userDashboardData);
        }
        console.log('');
        // Test 5: Admin accessing user list
        console.log('5️⃣ Testing admin access to user list...');
        const adminUsersResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/users`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${adminToken}`,
                'Content-Type': 'application/json',
        ...
        const userUsersResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/users`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${userToken}`,
                'Content-Type': 'application/json',
        ...
        const userUsersData = await userUsersResponse.json();
        if (userUsersResponse.status === 403) {
            ...
            console.log('❌ Regular user incorrectly allowed user list access:', userUsersData);
        }
        console.log('');
        // Test 7: Admin accessing permissions endpoint
        console.log('7️⃣ Testing admin access to permissions endpoint...');
        const adminPermissionsResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/permissions`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${adminToken}`,
                'Content-Type': 'application/json',
        ...
        const adminPermissionsData = await adminPermissionsResponse.json();
        if (adminPermissionsResponse.ok && adminPermissionsData.success) {
            console.log('✅ Admin permissions access successful');
            console.log('   - Permissions count:', adminPermissionsData.data?.permissions?.length);
            console.log('   - System admin access:', adminPermissionsData.data?.permissionGroups?.systemAdministration);
        }
        else {
        ...
        const userPermissionsResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/permissions`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${userToken}`,
                'Content-Type': 'application/json',
        ...
        if (userPermissionsResponse.ok && userPermissionsData.success) {
            ...
            console.log('   - Permissions count:', userPermissionsData.data?.permissions?.length);
            console.log('   - System admin access:', userPermissionsData.data?.permissionGroups?.systemAdministration);
        }
        else {
            console.log('❌ Regular user permissions access failed:', userPermissionsData);
        }
        console.log('');
        // Test 9: Admin accessing system health
        console.log('9️⃣ Testing admin access to system health...');
        const adminHealthResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/system/health`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${adminToken}`,
                'Content-Type': 'application/json',
        ...
        const adminRestartResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/system/restart`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${adminToken}`,
                'Content-Type': 'application/json',
        ...
        const adminRestartData = await adminRestartResponse.json();
        ...
        console.log('1️⃣1️⃣ Testing regular user access to system restart (should fail)...');
        const userRestartResponse = await (0, node_fetch_1.default)(`${BASE_URL}/admin/system/restart`, {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${userToken}`,
                'Content-Type': 'application/json',
        ...
        const userRestartData = await userRestartResponse.json();
        if (userRestartResponse.status === 403) {
            ...
            console.log('❌ Regular user incorrectly allowed system restart access:', userRestartData);
        }
        console.log('');
        ...
        console.log('✅ Regular users are properly restricted from admin endpoints');
