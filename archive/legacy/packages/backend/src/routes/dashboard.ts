...
router.get('/stats', async (req, res) => {
  try {
    ...
    const adminUsers = (await Database.query(
      'SELECT COUNT(*) AS c FROM users WHERE role = $1',
      ['admin']
    )) as { rows: Array<{ c: number }> };
    ...
    const templates = (await Database.query(
      'SELECT COUNT(*) AS c FROM projects WHERE is_active = 1 AND is_template = 1'
    )) as { rows: Array<{ c: number }> };

    // Module statistics - count ROS packages as modules
    const totalModules = (await Database.query(
      'SELECT COUNT(*) AS c FROM ros_packages WHERE is_active = 1'
    )) as { rows: Array<{ c: number }> };
    const totalDatasets = (await Database.query(
    ...
    res.json({
      ...
      data: {
        ...
        datasets: totalDatasets.rows[0]?.c ?? 0,
      },
    });
  } catch (error) {
