...
const IS_SQLITE = true; // Always use SQLite
...
router.get('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    let result: { rows: Array<Record<string, any>> };

    // SQLite-safe query: avoid JOINs and Postgres-specific ILIKE/arrays
    let query = `
      SELECT p.id, p.name, p.description, p.owner_id, p.is_active, p.is_template, p.created_at, p.updated_at,
             0 AS package_count
      FROM projects p
    ...
    if (typeof is_template === 'string') {
      query += ` AND p.is_template = ?`;
    ...
    result = (await Database.query(query, params as any[])) as {
...
router.get('/types', async (req: AuthRequest, res) => {
  try {
    const result = (await Database.query(
      'SELECT DISTINCT type FROM projects WHERE is_active = 1 ORDER BY type'
...
// GET /api/projects/templates - Get public templates (no auth required)
router.get('/templates', async (req: AuthRequest, res) => {
  try {
    const { search, category, type } = req.query;

    let query = `
      SELECT p.id, p.name, p.description, p.category, p.type, p.author, p.license, 
             p.tags, p.config, p.metadata, p.created_at, p.updated_at,
             0 AS package_count, 0 AS module_count
      FROM projects p
      WHERE p.is_active = 1 AND p.is_template = 1
    `;
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.log(
          `Attempting to clone from template: ${src.github_repo_owner}/${src.github_repo_name} to ${safeName}`
        );
        ...
        try {
          ...
          try {
            ...
            console.log(
              `Successfully created repo from template: ${newRepo.html_url}`
            );
          } catch (templateErr) {
            ...
            console.log(
              `Created new empty repo as fallback: ${newRepo.html_url}`
            );
          }
        }
      ...
      try {
        ...
        const safeName = cloneName
          ...
          .slice(0, 100);
        console.log(
          `Creating new repo for cloned project: ${safeName} (template had no GitHub repo info)`
        );
